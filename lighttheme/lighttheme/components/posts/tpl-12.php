<?php
$rtn .= '
<div class="blog-tpl-12 blog-odd-nor blog-nor format-' . $format . ' incount-' . $in_count . ' post-type-' . $this_post_type . ' blog-nor-with-bg-grey blog-nor-with-bg-grey-exp">
' . $featured_cur . '<div class="blog-l-text-out">
<div class="blog-l-text">
<h3>' . $currentcatname . '</h3>
<a href="' . get_permalink($post['ID']) . '">
<h2>' . $post['post_title'] . '</h2></a>' . $ex_txt . '<div class="blog-btns">
<a href="' . get_permalink($post['ID']) . '">' . $more_t_text . '</a>
</div>' . create_item_socials(get_permalink($post['ID']), $post['post_title']) . '</div>
</div><img class="blg-bg-col" ' . $blkBg . '  src="/wp-content/themes/lighttheme/images/menu-trans-req.png">
