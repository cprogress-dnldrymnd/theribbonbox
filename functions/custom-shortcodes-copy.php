...
add_shortcode('comment', function(){return'';});
...
function get_socials_function() {
    $rtn = '<div class="social-icons"> 
            ...
            </div><div class="social-icons social-icons-txt"> 
                 <a href="https://www.instagram.com/parenting.theribbonbox/" target="_blank" class="instagram">
                 @parenting.theribbonbox</a> | <a href="https://www.instagram.com/fertility.theribbonbox/" target="_blank" class="instagram1">@fertility.theribbonbox</a>
            </div><div class="social-icons social-icons-txt"> 
                 <a href="https://www.tiktok.com/@parenting.theribbonbox" target="_blank" class="ticktok">
                 @parenting.theribbonbox</a> | <a href="https://www.tiktok.com/@fertility.theribbonbox" target="_blank" class="ticktok1">@fertility.theribbonbox</a>
            </div>';
...
function get_giveaway_event_function($attr) {
    ...
    if (!empty($attr["post_id"])){ $post_id = $attr["post_id"]; }
    if (!empty($attr["style_format"])){ $style_format = $attr["style_format"]; }
    ...
    foreach($recent_posts as $post) :
        ...
        $addBorder = 'border-top: 5px solid '.$bcolour.';';
            ...
            if ($post_type == "giveaway-items"){ $head_text = "Win"; $button_text = "Enter Now"; $select_competition_date = get_field("select_competition_date", $post["ID"]);}
            if ($post_type == "events"){$head_text = "Event"; $button_text = "REGISTER HERE"; $select_competition_date = get_field("select_competition_date", $post["ID"]);}
            if ($post_type == "offer-items"){$head_text = "Offer"; $button_text = "Register Here"; $select_competition_date = get_field("offer_expiry_date", $post["ID"]);}
...
function create_item_socials($url, $title){
    $rtn = '<div class="post-share"><a class="post-share-btn" href="">Share</a><div class="post-share-items" style="display:none;"><div class="social-icons"> 
...
function display_insider_function() {
    $rtn = '
        <div class="post-follow-us insider-outer xxx">
...
function display_expertboxes_function() {
    $rtn = "<div class='post-follow-us post-follow-us-outer' style='margin-bottom:0 !important;'><div class='post-follow-us-inner'><h2>Are you an expert our readers will benefit from?</h2><hr>
                                <p>We collaborate with nutritionists, wellbeing coaches, fitness instructors, therapists, sex and relationship advisers, fertility clinics, fertility consultants, doctors and nurses, pediatricians, doulas, midwives, lactation consultants, parenting coaching, family therapists, experts in child development, teachers, SENCos, and so many more.</p>

<p>Our bank of coaches and specialists is always growing.<br>
If you'd like to discuss partnership opportunities, click <a href='mailto:hello@theribbonbox.com'>here</a> and we will be in touch.</p>
...
function blog_load_next_function($attr){
    ...
    $excludeids;
    if (!empty($attr["categoryid"])){ $categoryid = $attr["categoryid"]; }
    if (!empty($attr["posttype"])){ $posttype = $attr["posttype"]; }
...
$post_types = explode('/', $post_type);
    ...
    if (!empty($attr["page"])){ $isPage = true; }
