...
/**
 * Retrieves categories that have a specific page selected in the 'page_category' ACF relationship field.
 * * This function queries the term meta table to find any category where the serialized array
 * of the ACF relationship field contains the specified post ID.
 *
 * @param int|null $page_id The ID of the page to check against. Defaults to the current post ID.
 * @return array|WP_Error Array of WP_Term objects on success, empty array if none found, or WP_Error on failure.
 */
function dd_get_categories_linked_to_page($page_id = null)
{
    // Fallback to the current global post ID if no specific ID is provided
    if (! $page_id) {
        $page_id = get_the_ID();
    }

    // Return an empty array early if we don't have a valid ID (e.g., outside the loop without an ID)
    if (! $page_id) {
        return array();
    }
     ...
     * * Because ACF stores Relationship field data as a serialized array in the database, 
     * we use a 'LIKE' comparison. Wrapping the ID in double quotes ensures we match 
     * the exact ID in the serialized string (e.g., ...s:3:"123";...) and prevents false 
     * positives (like matching "12" when searching for "1").
     */
    $args = array(
        'taxonomy'   => 'category', // Change this if your field is on a custom taxonomy instead of default categories
        'hide_empty' => false,      // Keep false to return categories even if they have no standard posts assigned
        'meta_query' => array(
            array(
                'key'     => 'page_category',        // The ACF Field Name from your screenshot
                'value'   => '"' . $page_id . '"',   // Exact string match inside the serialized array
                'compare' => 'LIKE',
            ),
        ),
    );

    // Execute the term query and return the matching terms
